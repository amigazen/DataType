...
This will create the `DataType` executable in the Source directory.
...
This copies the executable to `../SDK/C/DataType`.
...
The build process:
1. Compiles `datatype.c` to `datatype.o` using SAS/C compiler
2. Links `datatype.o` with `sc:lib/c.o` and required libraries
3. Creates the `DataType` executable
...
1. Verify datatypes.library v45+ is installed
2. Check that the file exists and is readable
3. Ensure the file has a recognized datatype
